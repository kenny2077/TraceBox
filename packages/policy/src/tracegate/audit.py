...
# Keys in JSON objects to aggressively redact entirely (used as fallback)
SENSITIVE_PATTERNS = [
...
class AuditLogger:
    def __init__(self, log_dir: Optional[str] = None, server_command: Optional[str] = None,
        ...
        # Unified redaction engine — same path as response DLP
    ...
    def log_event(self, event_type: str, payload: Any):
        ...
        # Unified redaction: apply the same engine used for agent responses
        redacted_payload = self._redaction.redact(payload) if isinstance(payload, (dict, list)) else payload
    ...
    def log_tool_call(self, tool_name: str, arguments: Dict[str, Any], call_id: Any):
        # Redact arguments before logging to prevent secret leakage in audit
        safe_args = self._redaction.redact(arguments)
    ...
    def log_tool_result(self, call_id: Any, result: Any = None, error: Any = None):
        ...
        if result is not None:
            # Redact result before truncating to avoid leaking secrets in truncated preview
            safe_result = self._redaction.redact(result)
            payload["result"] = truncate_content(safe_result)
